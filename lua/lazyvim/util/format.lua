...
function M.formatexpr()
  if LazyVimx.has("conform.nvim") then
...
function M.info(buf)
  ...
  LazyVimx[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    LazyVimx.warn("No formatter available", { title = "LazyVimx" })
  end
end
...
function M.health()
  ...
  if has_plugin and not has_extra then
    LazyVimx.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in LazyVimx.",
      "",
