...
M.deprecated_extras = {
  ["lazyvim.plugins.extras.formatting.conform"] = "`conform.nvim` is now the default **LazyVimx** formatter.",
  ["lazyvim.plugins.extras.linting.nvim-lint"] = "`nvim-lint` is now the default **LazyVimx** linter.",
  ["lazyvim.plugins.extras.ui.dashboard"] = "`dashboard.nvim` is now the default **LazyVimx** starter.",
  ...
  ["lazyvim.plugins.extras.coding.mini-ai"] = "`mini.ai` is now a core LazyVimx plugin (again)",
  ...
  Either use `basedpyright`, or copy the [old extra](https://github.com/LazyVimx/LazyVimx/blob/c1f5fcf9c7ed2659c9d5ac41b3bb8a93e0a3c6a0/lua/lazyvim/plugins/extras/lang/python-semshi.lua#L1) to your own config.
  ]],
}
...
function M.fix_imports()
  Plugin.Spec.import = LazyVimx.inject.args(Plugin.Spec.import, function(_, spec)
    local dep = M.deprecated_extras[spec and spec.import]
    if dep then
      dep = dep .. "\n" .. "Please remove the extra from `lazyvim.json` to hide this warning."
      LazyVimx.warn(dep, { title = "LazyVimx", once = true, stacktrace = true, stacklevel = 6 })
      return false
    end
  end)
end
...
function M.fix_renames()
  Plugin.Spec.add = LazyVimx.inject.args(Plugin.Spec.add, function(self, plugin)
